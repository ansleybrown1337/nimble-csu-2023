...
```{r}
# calcDeps is a 'function generator' here creating a funciton specific for this regression
calcDeps_regr <- calcDeps(regrModel, c('b0', 'b1', 'sigma'))
# calcDeps_regr has a 'run' method b/c it was declared in nimbleFunction (line 49)
...
```{r, regr-optimize, warning=FALSE}
set.seed(1)
origWarn <- options()$warn
options(warn = -1)   ## turn off the warnings to keep the html cleaner
# AJ mod: put the warning turnoff in code block settings instead for Rmd
...
```{r, regr-optimize-grad, warning=FALSE}
# AJ note: turned warnings off in code block parameters
set.seed(1)
